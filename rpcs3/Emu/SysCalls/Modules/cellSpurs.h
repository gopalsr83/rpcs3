...
struct CellSpurs
{
	...
	static const uint size2 = 0x2000; // size of CellSpurs2
	...
	struct EventPortMux
	{
		atomic_t<u32> reqPending;   // 0x00
		be_t<u32> spuPort;          // 0x04
		be_t<u32> x08 ;             // 0x08
		be_t<u32> x0C;              // 0x0C
		be_t<u64> eventPort;        // 0x10
		atomic_t<u64> handlerList;  // 0x18
		u8 x20[0x80 - 0x20];        // 0x20
	};

	static_assert(sizeof(EventPortMux) == 0x80, "Wrong EventPortMux size");

	struct WorkloadInfo
	{
		vm::bptr<const void, 1, u64> addr; // 0x00 Address of the executable
	...
	union
	{
		...
		struct
		{
			...
			atomic_t<u64> spuPortBits;  // 0xDA8
	...
	union
	{
		...
		struct
		{
			union
			{
				struct
				{
					atomic_t<u16> events;         // 0x00 Event bits
					...
					u8 ppuWaitSlotAndMode;        // 0x06 Top 4 bits: Wait slot number of the blocked PPU thread, Bottom 4 bits: Wait mode of the blocked PPU thread
				...
				atomic_t<u64> eventFlagControl;   // 0x00 Event flag control
			};
